# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 15:46:27 2019
...
def largestProductInGrid(grid):
    ...
    for y in range(len(grid)):
        for x in range(len(grid[0])):
            if getDiagonalLeft(grid, x, y) > largestSum:
                largestSum = getDiagonalLeft(grid, x, y)
            if getDiagonalRight(grid, x, y) > largestSum:
                largestSum = getDiagonalLeft(grid, x, y)
            if getDown(grid, x, y) > largestSum:
                largestSum = getDiagonalLeft(grid, x, y)
            if getRight(grid, x, y) > largestSum:
                largestSum = getDiagonalLeft(grid, x, y)
    return largestSum
    
def getDiagonalLeft(grid, x, y):
    if (x < 3) or (y > len(grid) - 4):
        return 0
    else:
        return grid[y][x] * grid[y + 1][x - 1] * grid[y + 2][x - 2] * grid[y + 3][x - 3]

def getDiagonalRight(grid, x, y):
    if (x > len(grid[0]) - 4) or (y > len(grid) - 4):
        return 0
    else:
        return grid[y][x] * grid[y + 1][x + 1] * grid[y + 2][x + 2] * grid[y + 3][x + 3]
